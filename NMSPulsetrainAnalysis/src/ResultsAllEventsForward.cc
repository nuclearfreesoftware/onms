...
NMSMultiplicityResult NMSPulsetrainManager::ResultsAllEventsForward() {
  ...
  std::ofstream tmpdebug;
  std::string filename = "4-debug";
  ...
  for(int i = 0; i < processedevents.size(); i++) {
    ...
    while(j < processedevents.size()
    	  ...
    	  && ((processedevents[i] + mms.predelay) - processedevents[j] > epsilon) ) {
      j++;
    }

    while(j < processedevents.size()
    // ### processedevents[i] + predelay + (registerPeriod * registerLength) >= processedevents[j]
    	  && ((processedevents[i] + mms.predelay + (mms.registerPeriod * mms.registerLength) - processedevents[j]) > -epsilon) ) {
      oldj = j;
      j++;
      shiftcount++;
    }
