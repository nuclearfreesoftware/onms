...
NMSMultiplicityResult NMSPulsetrainManager::LLResultsSingleEventBackward(bool infinitegate) {
  ...
  for(size_t i = 0; i < eventgroups.size(); i++) {
    // First event always has no predecessors
    gateResult.addRA(0);
    noGateResult.addRA(0);
    for(size_t j = 1; j < eventgroups[i].size(); j++ ) {
      size_t k = j - 1;
	...
	gcount++;
      ...
      size_t remainingevents = eventgroups[i].size() - (pcount + gcount);
